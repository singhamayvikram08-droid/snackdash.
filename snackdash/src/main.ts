...
if (authForm) {
  ...
  /* Removed aggressive redirect to allow role switching */
}
...
function initLiveChat(chatRoomId: string, listenOrderId?: string) {
  ...
  if (chatForm) {
    ...
    photoBtn.style.cssText = 'background: none; border: none; color: var(--text-dim); cursor: pointer; padding: 0.5rem; display: flex; align-items: center; justify-content: center; transition: color 0.2s; flex-shrink: 0;';
    photoBtn.onmouseover = () => photoBtn.style.color = 'var(--primary)';
    photoBtn.onmouseout = () => photoBtn.style.color = 'var(--text-dim)';
...
function initScannerApp() {
  ...
  if (startScanBtn && qrReaderContainer) {
    startScanBtn.addEventListener('click', async () => {
      ...
      try {
        // @ts-ignore (Html5Qrcode comes from CDN)
        ...
        await html5QrCode.start(
          ...
          config,
          (decodedText: string) => {
            // Success: Fill search input and stop
            if (searchInput) {
  ...
  // Real-time notification for STAFF (Scanner)
  socket.on('new_order', (order: any) => {
    showToast(`🚨 NEW ORDER: ${order.customerName} - ORD-${order.id.split('-')[1]}`, 'success');
  ...
  if (chatForm) {
    chatForm.addEventListener('submit', async (e) => {
      ...
      // Add a "Direct Chat with Admin" helper button if not in staff mode
      if (!scannerStaffChatMode && chatMessages && !chatMessages.querySelector('.staff-connect-prompt')) {
        const promptDiv = document.createElement('div');
        promptDiv.className = 'chat-msg system staff-connect-prompt';
        promptDiv.innerHTML = `Need to talk to Admin? <button class="btn btn-outline" style="padding: 0.2rem 0.6rem; font-size: 0.7rem; border-color: var(--primary); color: var(--primary); margin-left: 0.5rem;" onclick="document.getElementById('chatInput').value='1278'; document.getElementById('chatForm').dispatchEvent(new Event('submit'));">Connect Now</button>`;
              ...
              <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 2rem; padding-bottom: 2rem; border-bottom: 1px solid rgba(255,255,255,0.1);">
                ...
                <div style="text-align: right;">
                  ...
                  <select id="scannerStatusSelect" class="order-status status-${data.status}" style="font-size: 1.2rem; padding: 0.5rem 1rem; border-radius: 2rem; font-weight: 800; cursor: pointer;">
                    <option value="Preparing" ${data.status === 'Preparing' || data.status !== 'Completed' ? 'selected' : ''}>Preparing</option>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (mobileMenuBtn && navLinks) {
    ...
    // Handle all variations of logout buttons
    const logoutButtons = ['logoutBtn', 'logoutBtnTop', 'adminLogoutBtn', 'scannerLogoutBtn', 'scannerLogoutBtnTop'];
